...
def constructBayesNet(gameState: hunters.GameState):
    ...
    observationDomain = list(range(maxManhattanDist + MAX_NOISE + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim_var in eliminationOrder:
            unjoined_factors, joined_factor = joinFactorsByVariable(factors, elim_var)

            if len(joined_factor.unconditionedVariables()) > 1:
                eliminated_factor = eliminate(joined_factor, elim_var)
                factors = unjoined_factors + [eliminated_factor]
            else:
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return random.choices(list(self.keys()), weights=self.values(), k=1)[-1]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            new_prob = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            self.beliefs[ghostPosition] *= new_prob
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_pos in self.allPositions:
            prior_prob = self.beliefs[old_pos]
            if prior_prob > 0:
                newPosDist = self.getPositionDistribution(gameState, old_pos)
                for p in newPosDist:
                    new_beliefs[p] += self.beliefs[old_pos] * newPosDist[p]
