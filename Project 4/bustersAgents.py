...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        most_likely_positions = [dist.argMax() for dist in livingGhostPositionDistributions]

        best_action = None
        min_dist_to_closest_ghost = float('inf')

        for action in legal:
            successor_position = Actions.getSuccessor(pacmanPosition, action)
            
            dist_to_closest_ghost = float('inf')
            for ghost_pos in most_likely_positions:
                dist = self.distancer.getDistance(successor_position, ghost_pos)
                if dist < dist_to_closest_ghost:
                    dist_to_closest_ghost = dist
            
            if dist_to_closest_ghost < min_dist_to_closest_ghost:
                min_dist_to_closest_ghost = dist_to_closest_ghost
                best_action = action
