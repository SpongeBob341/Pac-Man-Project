...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #list booleans one for each corner.
        corners_visited = [False, False, False, False]
        
        # Check if the starting position is on any of the corners.
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # next move hits a wall?
            if not self.walls[next_position[0]][next_position[1]]:
                
                # Start with the corners visited in the current state.
                next_corners_visited = list(corners_visited)
                
                # Check if this new position is one of the corners.
                if next_position == self.corners[0]:
                    next_corners_visited[0] = True
                if next_position == self.corners[1]:
                    next_corners_visited[1] = True
                if next_position == self.corners[2]:
                    next_corners_visited[2] = True
                if next_position == self.corners[3]:
                    next_corners_visited[3] = True
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    current_position, corners_visited = state   # tuple: (current_position, corners_visited_tuple)

    # list: corners that still need to be visited.
    unvisited_corners = [corners[i] for i, visited in enumerate(corners_visited) if not visited]

    if not unvisited_corners: return 0 # Goal Terminate

    # maximum Manhattan distance [current position-> unvisited corners]
